/**
 * Leinenchallenge Quiz Pitch - Footer Script V1.6.2
 * NEU: Einfacher Spinner (wartet auf Quiz-Daten, dann auto-advance)
 * Ziehgrad-Berechnung, Quiz-Daten, Timer System
 */

// ===== EINFACHER SPINNER (wartet auf Quiz-Daten) =====
(function() {
  'use strict';
  ...
  function showSpinner() {
    const spinner = document.querySelector('[data-loading-spinner="true"]');
    if (spinner) {
  ...
  function triggerNextStep() {
    ...
    if (nextButton) {
      console.log('🎯 Klicke Next Button (Daten geladen)');
      nextButton.click();
    } else {
      console.warn('⚠️ Kein [data-next-button] gefunden');
    }
  }
  ...
  function onDataReady() {
    ...
    console.log('✅ Quiz-Daten geladen – Spinner wird ausgeblendet');
    hideSpinner();
  ...
  // Spinner sofort anzeigen
  document.addEventListener('DOMContentLoaded', function() {
    showSpinner();
  });
  
  // Warten auf quizDataLoaded Event (wird von Quiz Data Management gefeuert)
  document.addEventListener('quizDataLoaded', function(event) {
    console.log('📊 Quiz-Daten geladen – bereite Auto-Advance vor');
    
    // Kurz warten damit alle data-Attribute gefüllt werden
    setTimeout(() => {
      onDataReady();
    }, 600);
  });
  
  // Fallback: Falls nach 15 Sekunden keine Daten kommen, trotzdem weiter
  setTimeout(() => {
    if (!spinnerDone) {
      console.warn('⏱️ Timeout: Quiz-Daten nicht geladen nach 15s – weiter ohne Daten');
      onDataReady();
    }
  }, 15000);
