...
window.showQuizAnswersInSpans = function() {
  ...
  // Häufigkeit Unruhe
  const unruheSpans = document.querySelectorAll('[data-quiz-answer="haeufigkeit_unruhe"]');
  unruheSpans.forEach(span => {
    span.textContent = getQuizAnswer('haeufigkeit_unruhe') || 'Unbekannt';
  });
  
  // Größter Stressor
  const stressorSpans = document.querySelectorAll('[data-quiz-answer="groesster_stressor"]');
  stressorSpans.forEach(span => {
    span.textContent = getQuizAnswer('groesster_stressor') || 'Unbekannt';
  });
  
  // Wie lange Problem
  const problemDauerSpans = document.querySelectorAll('[data-quiz-answer="wie_lange_problem"]');
  problemDauerSpans.forEach(span => {
    span.textContent = getQuizAnswer('wie_lange_problem') || 'Unbekannt';
  });
  
  // Läuft hinterher
  const hinterherSpans = document.querySelectorAll('[data-quiz-answer="laeuft_hinterher"]');
  hinterherSpans.forEach(span => {
    span.textContent = getQuizAnswer('laeuft_hinterher') || 'Unbekannt';
  });
  
  // Fordert Aufmerksamkeit
  const aufmerksamkeitSpans = document.querySelectorAll('[data-quiz-answer="fordert_aufmerksamkeit"]');
  aufmerksamkeitSpans.forEach(span => {
    span.textContent = getQuizAnswer('fordert_aufmerksamkeit') || 'Unbekannt';
  });
  
  // Besuch Reaktion
  const besuchSpans = document.querySelectorAll('[data-quiz-answer="besuch_reaktion"]');
  besuchSpans.forEach(span => {
    span.textContent = getQuizAnswer('besuch_reaktion') || 'Unbekannt';
  });
  
  // Geräusch Reaktion
  const geraeuschSpans = document.querySelectorAll('[data-quiz-answer="geraeusch_reaktion"]');
  geraeuschSpans.forEach(span => {
    span.textContent = getQuizAnswer('geraeusch_reaktion') || 'Unbekannt';
  });
  
  // Anfang Spaziergang
  const spaziergang1Spans = document.querySelectorAll('[data-quiz-answer="anfang_spaziergang"]');
  spaziergang1Spans.forEach(span => {
    span.textContent = getQuizAnswer('anfang_spaziergang') || 'Unbekannt';
  });
  
  // Nach Spaziergang
  const spaziergang2Spans = document.querySelectorAll('[data-quiz-answer="nach_spaziergang"]');
  spaziergang2Spans.forEach(span => {
    span.textContent = getQuizAnswer('nach_spaziergang') || 'Unbekannt';
  });
  
  // Alleine bleiben
  const alleineSpans = document.querySelectorAll('[data-quiz-answer="alleine_bleiben"]');
  alleineSpans.forEach(span => {
    span.textContent = getQuizAnswer('alleine_bleiben') || 'Unbekannt';
  });
  
  // Ruhezeit Stunden
  const ruhezeitSpans = document.querySelectorAll('[data-quiz-answer="ruhezeit_stunden"]');
  ruhezeitSpans.forEach(span => {
    span.textContent = getQuizAnswer('ruhezeit_stunden') || 'Unbekannt';
  });
  
  // Rückzugsort
  const rueckzugsortSpans = document.querySelectorAll('[data-quiz-answer="rueckzugsort"]');
  rueckzugsortSpans.forEach(span => {
    span.textContent = getQuizAnswer('rueckzugsort') || 'Unbekannt';
  });
  
  // Tagesablauf Struktur
  const tagesablaufSpans = document.querySelectorAll('[data-quiz-answer="tagesablauf_struktur"]');
  tagesablaufSpans.forEach(span => {
    span.textContent = getQuizAnswer('tagesablauf_struktur') || 'Unbekannt';
  });
  
  // Wer bestimmt
  const bestimmtSpans = document.querySelectorAll('[data-quiz-answer="wer_bestimmt"]');
  bestimmtSpans.forEach(span => {
    span.textContent = getQuizAnswer('wer_bestimmt') || 'Unbekannt';
  });
  
  // Ignorieren können
  const ignorierenSpans = document.querySelectorAll('[data-quiz-answer="ignorieren_koennen"]');
  ignorierenSpans.forEach(span => {
    span.textContent = getQuizAnswer('ignorieren_koennen') || 'Unbekannt';
  });
  
  // Wie lange Kampf
  const kampfSpans = document.querySelectorAll('[data-quiz-answer="wie_lange_kampf"]');
  kampfSpans.forEach(span => {
    span.textContent = getQuizAnswer('wie_lange_kampf') || 'Unbekannt';
  });
  
  // Überfordert
  const ueberfordertSpans = document.querySelectorAll('[data-quiz-answer="ueberfordert"]');
  ueberfordertSpans.forEach(span => {
    span.textContent = getQuizAnswer('ueberfordert') || 'Unbekannt';
  });
  
  // Auswirkung Beziehung
  const beziehungSpans = document.querySelectorAll('[data-quiz-answer="auswirkung_beziehung"]');
  beziehungSpans.forEach(span => {
    span.textContent = getQuizAnswer('auswirkung_beziehung') || 'Unbekannt';
  });
  
  // Sorgen Gesundheit
  const gesundheitSpans = document.querySelectorAll('[data-quiz-answer="sorgen_gesundheit"]');
  gesundheitSpans.forEach(span => {
    span.textContent = getQuizAnswer('sorgen_gesundheit') || 'Unbekannt';
  });
  
  // Bereits versucht
  const versuchtSpans = document.querySelectorAll('[data-quiz-answer="bereits_versucht"]');
  versuchtSpans.forEach(span => {
    span.textContent = getQuizAnswer('bereits_versucht') || 'Unbekannt';
  });
  
  // Erfolg Versuche
  const erfolgSpans = document.querySelectorAll('[data-quiz-answer="erfolg_versuche"]');
  erfolgSpans.forEach(span => {
    span.textContent = getQuizAnswer('erfolg_versuche') || 'Unbekannt';
  });
  
  // Wissen Stress
  const wissenSpans = document.querySelectorAll('[data-quiz-answer="wissen_stress"]');
  wissenSpans.forEach(span => {
    span.textContent = getQuizAnswer('wissen_stress') || 'Unbekannt';
  });
  
  // Ziel 6 Wochen
  const zielSpans = document.querySelectorAll('[data-quiz-answer="ziel_6_wochen"]');
  zielSpans.forEach(span => {
    span.textContent = getQuizAnswer('ziel_6_wochen') || 'Unbekannt';
  });
  
  // Wichtigster Punkt
  const wichtigsterSpans = document.querySelectorAll('[data-quiz-answer="wichtigster_punkt"]');
  wichtigsterSpans.forEach(span => {
...
window.showConditionalContent = function() {
  ...
  allContentElements.forEach(element => {
    ...
    // ✅ VEREINFACHT: Nur Quiz-Antworten
...
function getEmailFromStorage() {
  ...
  // ✅ GEÄNDERT: AST-spezifische localStorage Keys
  const email = localStorage.getItem('email') || 
               localStorage.getItem('ast_useremail') ||  // ← NEU
               localStorage.getItem('asp_useremail') ||  // ← NEU
...
function loadLocalDataImmediately() {
  // ✅ GEÄNDERT: AST-spezifische Keys zuerst
